	...
	ss.initClass($UI_TableViewModel, $asm, {
		getCartesianArray: function() {
			...
			while (i < this.data().length) {
				...
				$t1.x = md + this.data()[i]._MD * Math.sin(this.data()[i]._Z) * Math.cos(this.data()[i]._A);
				$t1.y = md + this.data()[i]._MD * Math.sin(this.data()[i]._Z) * Math.sin(this.data()[i]._A);
				$t1.z = md + this.data()[i]._MD * Math.cos(this.data()[i]._Z);
				cartesianarray.push($t1);
				md += this.data()[i]._MD;
				i++;
			}
			i = 0;
			while (i < cartesianarray().length) {
